...
class TrainSynth90k(object):
    ...
    def __init__(self, model):
        ...
        self.device = torch.device(
            "cuda:0" if torch.cuda.is_available() else "cpu") # để ngoài đây bởi vì ta muốn setting device trên GUI
    ...
    def train_network(self, cfg):
        ...
        elif cfg['OPTIMIZER']['optim_name'] == 'RMS':
...
#=============finetuning icdar
        train_icdar_special_dataset = Icdar15Dataset(
            root_dir    = 'text-reg-data/icdar',
            mode        = 'train_icdar_special',
            img_height  = cfg['DATASET']['resize']['height'], 
            img_width   = cfg['DATASET']['resize']['width'] )

        
        
        trainset = ConcatDataset([train_synth_dataset, train_special_character_dataset,train_anotherexp_dataset,train_icdar_special_dataset])
...
#=============finetuning icdar
        ...
        valid_icdar_special_dataset = Icdar15Dataset(
            root_dir    = 'text-reg-data/icdar',
            mode        = 'val_icdar_special',
            img_height  = cfg['DATASET']['resize']['height'], 
            img_width   = cfg['DATASET']['resize']['width'] )

        validset = valid_icdar_special_dataset
        #validset = ConcatDataset([valid_synth_dataset, valid_special_dataset,valid_icdar_special_dataset])
...
#====== test file
        ...
        images_synth_path_list = txt_into_list(cfg)[:500] # truyền vào annotation test file thì nó tự tự láy ra path file  #Synthtext annotation_test 500 anh
        images_special_character_path_list = get_images('text-reg-data/base-data/special_character10')[800:1000]
        images_icdar13_path_list = get_images('text-reg-data/icdar/icdar13/test')  # icdar13 test
        images_icdar15_path_list = get_images('text-reg-data/icdar/icdar15/test') # icdar15 test
        images_icdar17_val_special_path_list = get_images('text-reg-data\icdar\icdar17_special\icdar17_valid_special') # icdar17 val
        images_path_list = images_synth_path_list + images_special_character_path_list + images_icdar13_path_list + images_icdar15_path_list + images_icdar17_val_special_path_list
        ...
        #self.visualize_augmentations(trainset)
    ...
    def training(self, train_loader, valid_loader,predict_dataset, criterion, scheduler,valid_decode_method,valid_beam_size, num_training_steps: int = 1000, 
        ...
        if os.path.exists(os.path.join(savedir, resume)):
            print("LOAD MODEL")
            self.model, self.optimizer, step,scheduler ,best_score = self.load_model(self.model,
                                                                            self.optimizer,
                                                                            scheduler,
                                                                            os.path.join(savedir, resume))                                          

        # if os.path.exists('best_model.pth'):
        #     print("LOAD MODEL")
        #     self.model, self.optimizer, step, best_score = self.load_model(self.model,
        #                                                                     self.optimizer,
        #                                                                     'best_model.pth')                                          
        ...
        while train_mode:
            ...
            for train_data in train_loader: # 1 batchsize train_data = 1 step
                ...
                self.metric_monitor.update("Loss", loss, self.optimizer.param_groups[0]['lr'])

                
                if use_wandb:
                    wandb.log({
                        'lr': self.optimizer.param_groups[0]['lr'],
                        'loss' : self.metric_monitor.getdic},
                        step=step)
                ...
                if ((step+1) % eval_interval == 0 and step != 0) or (step+1) == num_training_steps:
                    ...
                    self.model.train() # ?? tại sao chỗ này 
    ...
    def visualize_augmentations(self, dataset, samples=2):
